...
`research/wall-correspondence` workstream (doc numbers theirs).

## The instrument: the physics ↔ filter isomorphism

| tier | status | where |
|---|---|---|
| state (trust = precision; ℝ, U(1), S³) | [x] | 0065, 0096–0099 |
| transfer (Kalman predict = polar semigroup) | [x] | 0098 |
| cycle (RG blocking = predict/update) | [x] | 0098 |
| curvature (β = (1−1/b²)τ²/6; the running S³ filter) | [x] | 0099 |
| innovation (boundary = the channel, capacity = ln N) | [x] | 0100 (abelian tier) |
| bank (marginalization; dressed vacuum measured) | [x] | 0097, 0101–0102 |
| time (Euclidean = smoother; Lorentzian = filter) | [x] | 0103 |
| network (lattice = bank of S³ filters; smoother predicts dressing) | [x] | 0104 |
| gauge quotient (= identifiability quotient) | [x] | 0104 |
...
| component | physics side | filter side | status |
|---|---|---|---|
| Λ path (budget = Stokes; Λ quantized) | 0080 | — | [x] |
| Lorentzian kinematics (N ≡ 1 mod 4; reality = Frobenius) | 0081 | — | [x] |
| Path C: area law, Unruh, capacity, two ledgers | 0082–0086 | — | [x] |
| RG flow, running τ, dimensional transmutation | 0092–0093, 0099 | F2 dissolved (physics got there first) | [x] |
...
## 0158 / 0168 — the Barrett–Crane flag, tested and retired

0166's flag rested on a collapsed distinction: **balanced
representations are BC's simplicity *condition*; intertwiner-
independence is BC's *defect*.** Having the first is not evidence of
the second.

Whether a lattice gauge theory is intertwiner-blind is not a choice
made in the weight. The character expansion's **link integrals** are
themselves projectors onto invariants, and in 4D each link is shared
by four plaquettes — a 4-valent node. Measured rank of that projector:
**2, 3, 2, 2 against dim Inv = 2, 3, 2, 2.** Rank = dim Inv, never 1.
The intertwiner space is generated by the **measure**. Reps gated at
3e−16 (a first attempt had the symmetric-power indexing reversed).

For Spin(4) the balanced condition acts face by face and the link
integral factorises: **36 intertwiner states per link** at the top
spin. BC additionally projects onto a *single* element — this does
not.

**Filter side (lucid 0051):** an intertwiner is the shape of a closed
node modulo rotation, dimension 2n − 6 (measured 0, 2, 4, 6). At
3-valence a point; at 4-valence two-dimensional — a tetrahedron's face
areas do not fix its shape. Blindness would cost **ln 6 = 1.79 nats
per node**. And the capacity principle counts states, so it *forbids*
the blind form rather than forcing it, leaving κ untouched either way.

**Retires 0167's "most serious open item."** But: the propagator has
still never been measured, and a residual stands — this imposes
